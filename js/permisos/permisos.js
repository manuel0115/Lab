...
var tablaAnalisis = $("#tblAnalisis").DataTable({
  ajax: "permisos/cargarDatosPermisos",
  type: "POST",
  columns: [
  ...
  buttons: [
    {
      ...
      text:
        '<span class="btn-label"><i class="fa fa-plus"></i></span>&nbsp;Agregar Permisos',
      action: function (nButton, oConfig, oFlash) {
        $(".modal_usuarios .modal-content").load(
          "permisos/getModalPermisos",
          function () {
...
$(".modal_usuarios .modal-content").on("click","#btn_guardar_permiso",function(){
 // alert();
...
$("#tblAnalisis").on("click", ".btn_editar_permisos", function () {
  
  id=btoa(btoa(btoa($(this).attr("id"))));

  $(".modal_usuarios .modal-content").load("permisos/getModalPermisos/"+id,function () {
